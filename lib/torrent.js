...
class Torrent extends EventEmitter {
    ...
    this.store = new ImmediateChunkStore(
      this._preloadedStore || new this._store(this.pieceLength, {
        ...
        files: this.files.map(file => {
          ...
          stub.path = path.join(this.path, file.path)
          stub.length = file.length
          stub.offset = file.offset
          return stub;
        }),
        length: this.length,
        name: this.infoHash
      ...
      this.store.get(index, getOpts, (err, buf) => {
        ...
        sha1(buf, hash => {
          ...
          if (hash === this._hashes[index]) {
            ...
            this.emit('piece-invalid', index, this.pieces.length)
          }
          cb(null)
        })
      })
  ...
  _markVerified (index) {
    ...
    this.emit('piece-verified', index, this.pieces.length)
  }
